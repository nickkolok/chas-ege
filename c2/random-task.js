...
var initializeStatsTable = function() {
	...
	for (var i = 1; i <= nabor.nZad; i++) {
		...
		try {
			...
			// FIXME: Надо бы проверять, но нужно ли?
		}

		rowIndexes.append(createDataIndexes(i, title, nabor.name));
		rowRight.append(createDataRight(i, title));
		rowStats.append(createDataStats(i, title));
