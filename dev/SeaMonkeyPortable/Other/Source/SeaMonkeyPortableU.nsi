...
Section "Main"
	...
	SettingsFound:
		...
		${If} ${FileExists} "$EXEDIR\LupoApp.ini"
			...
			${ReadINIStrWithDefault} $1 "$SETTINGSDIRECTORY\SeaMonkeyPortableSettings.ini" "SeaMonkeyPortableSettings" "InvalidPackageWarningShown" "0.0.0.0"
			...
			${If} $2 == 1
				...
				WriteINIStr "$SETTINGSDIRECTORY\SeaMonkeyPortableSettings.ini" "SeaMonkeyPortableSettings" "InvalidPackageWarningShown" $0
			${EndIf}
		${EndIf}
	...
	GPGEnvironment:
		...
		IfFileExists "$GPGPATHDIRECTORY\gpg.exe" "" LaunchNow
