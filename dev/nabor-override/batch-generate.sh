...
# 1. Запрос начального номера
if [ -z "$START" ]; then
    read -p "Введите начальный номер варианта: " START
fi
if ! [[ "$START" =~ ^[0-9]+$ ]]; then
...
# 2. Запрос количества
if [ -z "$COUNT" ]; then
    read -p "Введите количество вариантов для генерации: " COUNT
fi
...
# 3. Запрос количества подвариантов (с дефолтом 5)
if [ -z "$CV" ]; then
