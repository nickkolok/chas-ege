...
$(document).ready(function(){$('body').prepend('<style>.math_ed_butt {background:white; border:1px solid black; margin:2px; padding:2px 3px;cursor:pointer; border-radius:2px; display:inline-block; height:18px;vertical-align:middle; line-height:18px} .math_ed_butt span{} .meb_big {height:30px;line-height:30px}</style>');});
    mbuttons = [
    ...
    ['[', '\\left [ \\begin{align} &a,\\\\&b. \\end{align} \\right '],
    ['<img src="/formula/03/033b571c237d78ae1c9908427fdf52ce.png" class="tex" style="position:relative;top:-4px" width="6">','\\frac{a}{b}'],
    ['&radic;<span style="text-decoration:overline">x</span>', '\\sqrt{x}'],
    ['<sup>3</sup>&radic;','\\sqrt[3]{x}'],
    ['x<sup>2</sup>', '^2'],
    ['x<sub>2</sub>', "_2"],
    ...
    ['∠','\\angle'],
    ['∠ABC','\\widehat{ABC}'],
    ['<img src="/formula/04/041c351f00a0d7e2571b62210e1dc282.png" class="tex" style="position:relative;top:-12px" width="15">','\\underset{...}{\\mathop{\\Leftrightarrow}}'],
    ];
...
prepare_editor = function(){
    $('.math_editor').each(function(index,elem){
        ...
        special[2] = $('<div class="math_ed_butt">Таблица 1</div>').data('ta', ta).click(function(){
            var cols = prompt('Количество столбцов', 1);
            var rows = prompt('Количество строк', 1) - 1;
            var html = '<tr>';
            ...
            insertAtCaret($(this).data('ta')[0], '\n\n<table style="margin:auto">'+html+'</table>\n\n');
            jq_preview($(this).data('ta').attr('id').replace('editor_', ''), true)
        });
        special[3] = $('<div class="math_ed_butt">Таблица 2</div>').data('ta', ta).click(function(){
            var cols = prompt('Количество столбцов', 1);
            var rows = prompt('Количество строк', 1) - 1;
            var html = '<tr>';
            for (var i=0; i<cols; i++)
            ...
            for (var j=0; j<rows; j++){
                ...
                for (var i=0; i<cols; i++)
                    if (i==0)
                        ...
                        html += '<td style="text-align:center">'+(j+2)+'.'+(i+1)+'</td>';
                html+='</tr>';
            }
            insertAtCaret($(this).data('ta')[0], '\n\n<table style="margin:auto">'+html+'</table>\n\n');
            jq_preview($(this).data('ta').attr('id').replace('editor_', ''), true)
