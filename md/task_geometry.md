...
**Важно:** Если в задаче нет ни одной буквы (ни в условии, ни на чертеже),
то `NAtask.modifiers.variativeABC()` вызывать **не нужно**, даже если есть чертёж!
Например, в задачах на отношение объёмов или площадей правильных многогранников, где вершины не обозначены буквами, этот модификатор не применяется.
...
В задачах по стереометрии ответ часто содержит корни (например, $a\sqrt{3}$ для диагонали куба).
Чтобы привести ответ к целому числу в формате ЕГЭ, используйте модификатор `NAtask.modifiers.multiplyAnswerBySqrt(n)`.
Он добавит в условие фразу вроде «Ответ умножьте на $\sqrt{3}$» и домножит ожидаемый ответ.
Подробнее см. в `md/task_modifiers.md`.
