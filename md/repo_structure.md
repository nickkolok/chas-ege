...
| `Gruntfile.js` | Вся сборка. Обычно трогать не надо. |
...
- `rusege2022` — ЕГЭ русский (едва начат, дремучие черновики)
...
Обычно сюда лезть не надо, если не просят специально.

- `scripts/` — вспомогательные скрипты (fast_set и др.)
- `parser/` — парсер (перенесён из lib)
- `txtdocs/` — текстовые доки (vocabulary.txt, random.txt)
...
Промежуточные файлы → `build/`, итог → `dist/`.
