	...
	for (var i = 0; i < nodeCount; i++){
		for (var j = 0; j < nodeCount; j++){
			if(i!=j && nodePathArray[i][j]=='+'){
				var startNode = new Complex(nodeXarray[i], nodeYarray[i]);
				var endNode = new Complex(nodeXarray[j], nodeYarray[j]);
				var edgeLength = new Complex().sum(startNode, endNode.minus()).norma();
