(function () {
    ...
    retryWhileError(function () {
        ...
        let item = sklonlxkand(['ручка', 'тетрадь', 'линейка', 'папка', 'скрепка','кисть','обложка', 'закладка', 'точилка'].iz());
        ...
        NAtask.setTask({
            text: 'В магазине канцтоваров продаётся ' + chislitlx(total, item, '$') + ': ' + '$' + c1 + '$ ' + colorsGen[0] + ', $' + c2 + '$ ' + colorsGen[1] + ', $' + c3 + '$ ' + colorsGen[2] + 
            ', ' + 'остальные — ' + colorsNomPl[0] + ' и ' + colorsNomPl[1] + ', их поровну' + '. ' +
                'Найдите вероятность того, что случайно выбранная в этом магазине ' + item.ie + ' будет ' + colorsInstr.join(' или ') + '.',
            answers: probability,
        });
    }, 1000);
})();
