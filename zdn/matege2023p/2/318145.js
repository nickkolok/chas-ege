(function() {
	retryWhileError(function() {
		...
		let paint1 = function(ctx) {
			...
			//образующие
			ctx.drawLine(10, 23, 150, 300);
			ctx.drawLine(300 - 10, 23, 150, 300);
			//эллипс
			ctx.beginPath();
			ctx.ellipse(150, 20, 20, 140, Math.PI / 2, 0, 2 * Math.PI);
			ctx.stroke();
			ctx.closePath();
			...
			ctx.closePath();
			ctx.beginPath();
			ctx.ellipse(150, 300 * frac, 10, 140 * (1 - frac) + add, Math.PI / 2, 0, 2 * Math.PI);
			ctx.fill();
			ctx.closePath();
			...
			ctx.beginPath();
			ctx.setLineDash([5, 5]);
			ctx.ellipse(150, 300 * frac, 10, 140 * (1 - frac) + add, Math
