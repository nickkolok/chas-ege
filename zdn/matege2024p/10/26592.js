(function() {
	retryWhileError(function() {
		...
		let detail = sklonlxkand(['деталь','заготовка','продукт','предмет','горшок','беляш','пирожок','бутерброд','кувшин','молоток','инструмент'].iz());
		let rab = sklonlxkand(['рабочий','сотрудник','работник','мастер'].iz());

		NAtask.setTask({
			text: ['Заказ на','Задание на изготовление'].iz()+' '+chislitlx(A, detail.ie)+' первый '+rab.ie+' выполняет на '+n+' часов быстрее, чем второй. '+
				'Сколько '+detail.rm+' в час делает второй '+rab.ie+', если известно, что первый за час делает на '+chislitlx(b, detail.ie)+' больше.',
			answers: x,
			authors: ['Aisse-258']
		});
