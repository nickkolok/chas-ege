(function() {
	retryWhileError(function() {
		...
		let B = sl(10,600);//кол-во деталей2
		genAssert(Math.abs(A-B)<Math.min(A,B)/2,'Разница "заказа" слишком большая: A:'+A+' B: '+B);
		let k = sl(1,A/4-1,0.01);//кол-во деталей (разница)
		genAssert(k<30,'Разница в час слишком большая: '+k);
		let x = sl(1,100,0.01);//кол-во деталей (ответ)
		genAssert(k<x,'Разница больше изготовления: '+x);
		let n = (B*k+(B-A)*x)/(x*(x+k));//время изготовления (разница)
		genAssertZ1000(n,'Время слишком дробное: '+n);
		genAssert(n<24 && n>0,'Время > 24 ч или <=0: '+n);
