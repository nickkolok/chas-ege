(function () {
	retryWhileError(function () {
		...
		genAssert(![{ x: coordVectorA[0], y: coordVectorA[1] }, { x: coordVectorA[2], y: coordVectorA[3] }, { x: coordVectorB[0], y: coordVectorB[1] }, { x: coordVectorB[2], y: coordVectorB[3] }].mt_otrPeres(), 'Вектора A и B пересекаются');
		genAssert(![{ x: coordVectorA[0], y: coordVectorA[1] }, { x: coordVectorA[2], y: coordVectorA[3] }, { x: coordVectorC[0], y: coordVectorC[1] }, { x: coordVectorC[2], y: coordVectorC[3] }].mt_otrPeres(), 'Вектора A и C пересекаются');
		genAssert(![{ x: coordVectorB[0], y: coordVectorB[1] }, { x: coordVectorB[2], y: coordVectorB[3] }, { x: coordVectorC[0], y: coordVectorC[1] }, { x: coordVectorC[2], y: coordVectorC[3] }].mt_otrPeres(), 'Вектора B и C пересекаются');
		...
		let condition = (coeffs[0][0].texrndfrac(coeffs[0][1]) + '\\vec{' + letter[0] + '}\\cdot(') + (coeffs[1][0].texrndfrac(coeffs[1][1]) + '\\vec{' + letter[1] + '}' + '+'.esli(coeffs[2][0] > 0) + coeffs[2][0].texrndfrac(coeffs[2][1]) + '\\vec{' + letter[2] + '})');
