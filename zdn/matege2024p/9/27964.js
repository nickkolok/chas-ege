(function() { 'use strict'; retryWhileError(function() {
    ...
    let decorPreFormula = [
        `(в ${[`км`,`м`][rand]}) от ${the_vehicleRacingOnRoad.re} до ${the_humanSettlementDestination.re} вычисляется по формуле`,
    ...
    let decorQuestion = [
        ...
        ${the_humanSettlementDestination.re} на ${S} ${[`км`,`м`][rand]}`,
        `наибольшее время, в течение которого ${the_vehicleRacingOnRoad.ie} будет находиться в зоне функционирования сотовой связи, 
        если оператор гарантирует покрытие на расстоянии не далее чем в ${S} ${[`км`,`м`][rand]} от ${the_humanSettlementDestination.re}`
    ].iz();

	NAtask.setTask({
		text: `${the_vehicleRacingOnRoad.ie.toZagl()}, движущийся по ${the_humanSettlementDestination.de} со скоростью $v_0 = ${v0} \\ \\mbox{${[`км`,`м`][rand]}}/\\mbox{${[`ч`,`с`][rand]}}$, выезжает из него и сразу 
        после выезда начинает разгоняться с постоянным ускорением $a = ${a} \\ \\mbox{${[`км`,`м`][rand]}}/\\mbox{${[`ч`,`с`][rand]}}^2$. Расстояние ${decorPreFormula} 
        $S = v_0 t + \\frac{a t^2}{2} $, где $t$ – время в ${[`часах`,`секундах`][rand]}${decorPostFormula}. ${the_orderToFind.toZagl()} ${decorQuestion}. Ответ дайте в ${[`минутах`,`секундах`][rand]}.`,
		answers: t2*60,
