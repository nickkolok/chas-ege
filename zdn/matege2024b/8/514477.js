(function () {
	...
	retryWhileError(function () {
		...
		let city = sklonlxkand(['Москва', 'Санкт-Петербург', 'Екатеринбург', 'Архангельск', 'Мурманск', 'Норильск', 'Махачкала', 'Сочи', 'Краснодар'].iz(3));
		...
		let correct = [
			...
			`${city[0].ie} теплее ${city[1].re} на ${colder}°C.`,
			`${city[2].ie} теплее ${city[0].re} на ${hotter}°C.`
		];
		let wrong = [
		...
		NAtask.setTask({
			text: `В некоторый момент температура воздуха в ${city[0].pe} была равна ${tempA}°C. В этот же момент в ${city[1].pe} было на ${colder}°C холоднее, чем в ${city[0].pe}, а в ${city[2].pe} на ${hotter}°C теплее, чем в ${city[0].pe}.` +
