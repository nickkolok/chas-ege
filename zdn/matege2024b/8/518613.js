(function () {
	...
	retryWhileError(function () {
		...
		let wrong = [
			...
			'Найдётся ' + chislitlx(drawn, 'шарик', '$', (' '+colorsRM[1]+' ')) + ' с рисунками.',
		...
		NAtask.setTask({
			text: name.de + ' на день рождения подарили ' + chislitlx(total, 'шарик', '$v') + ', из них $' + count1 + '$ ' + colorsRM[0] + ', а остальные — ' + colorsVM[1] + '. ' + name.ie +
				' на $' + drawn + '$ случайных шариках нарисовала рисунки маркером, чтобы подарить '+['маме', 'папе', 'брату', 'сестре', 'бабушке', 'дедушке'].iz(drawn).joinWithConjunction() +'. Выберите все утверждения, которые будут ' + (rand ? 'неверны' : 'верны') +
