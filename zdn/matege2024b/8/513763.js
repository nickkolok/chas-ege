(function () {
	...
	retryWhileError(function () {
		...
		let wrong = [
			...
			`Каждый день, отличный от ${days[0].re} и ${days[2].re}, ученикам  ${classNumber} «${letter}» ${item.ve} можно в школу не брать.`,
			`${item.ie.toZagl()} ${['нужен','нужна'][item.rod]} каждый день.`,
			`Только по ${days[0].dm} и ${days[2].dm} требуется ${item.ie}.`
		];

		NAtask.setTask({
			text: `В  ${classNumber} «${letter}» классе ${subject.ie} по расписанию по ${days[0].dm} и ${days[1].dm}. Каждый ученик должен приносить ${item.ve} на каждый урок ${subject.re}. На других уроках ${item.ie} не требуется. ` +
