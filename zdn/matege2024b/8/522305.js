(function () {
	...
	retryWhileError(function () {
		...
		let correct = [
			...
			`Максимум ${chislitlx(maxBoth,'машина','$')} требуют замены обеих деталей.`,
		...
		let wrong = [
			...
			`Менее ${chislitlx(minNone,'машина','r$')} не требуют никакой замены.`,
			`Максимум ${chislitlx(maxBoth - 1,'машина','$')} требуют замены обеих деталей.`
		];

		if (pads > filter) {
			wrong.push(`Если в машине нужно менять колодки, то фильтр тоже нужно менять.`);
		} else if (pads < filter) {
			wrong.push(`Если в машине нужно менять фильтр, то колодки тоже нужно менять.`);
		}

		if (pads !== filter) {
			wrong.push(`Колодки и фильтр всегда меняют вместе.`);
		}
