(function () {
	...
	retryWhileError(function () {
		...
		let correct = [
			...
			`Высота любого жилого дома в ${town} не меньше ${chislitlx(minHeight - sl(1,2), 'метр','r')}.`,
			`Максимальная высота меньше ${chislitlx(maxHeight, 'метр','r')}.`,
			`Минимальная высота превышает ${chislitlx(minHeight - sl(1,2), 'метр')}.`
		];
		let wrong = [
			`В ${town} есть жилой дом высотой ${chislitlx(maxHeight, 'метр')}.`,
			`Разница в высоте любых двух домов больше ${chislitlx(maxHeight - minHeight + sl(5,10), 'метр','r')}.`,
			`Существует дом высотой ${chislitlx(minHeight - sl(1,2), 'метр')}.`,
			`Максимальная высота равна ${chislitlx(maxHeight, 'метр')}.`
		];

		NAtask.setTask({
			text: `В ${town} ${letter} всего ${houseCount} жилых домов. Высота каждого дома меньше ${chislitlx(maxHeight, 'метр','r')}, но не меньше ${chislitlx(minHeight, 'метр','r')}. Выберите одно или несколько утверждений, которые ` + (rand ? 'неверны' : 'верны') + ` при указанных условиях. В ответе запишите номера выбранных утверждений без пробелов, запятых и других дополнительных символов. Если ответов несколько, записывайте их номера в порядке возрастания.`,
