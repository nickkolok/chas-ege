(function () {
	...
	retryWhileError(function () {
		...
		let correct = [
			...
			`Если все, кто знают ${lang[0]}, также знают ${lang[1]}, то двуязычных — ${maxBoth}.`,
			`Знающих только ${lang[0]} на ${chislitM(secondLang - firstLang, 'человека', 'человека', 'человек')} меньше, чем тех, кто знает только ${lang[1]}.`,
		];
		let wrong = [
			`Нет ни одного человека в этой фирме, знающего и ${lang[0]}, и ${lang[1]} языки.`,
			`Если человек из этой фирмы знает ${lang[1]} язык, то он знает и ${lang[0]}.`,
			`Число двуязычных обязательно равно ${Math.floor((firstLang + secondLang) / 2)}.`,
			`${lang[1].toZagl()} знают только те, кто не изучал ${lang[0]} язык.`,
		];

		NAtask.setTask({
			text: `В фирме работает ${chislitM(total, 'человек', 'человека', 'человек')}, из них ${chislitM(firstLang, 'человек', 'человека', 'человек')} ${chislit(firstLang, 'знает', 'знают', 'знают')} ${lang[0]} язык, а ${chislitM(secondLang, 'человек', 'человека', 'человек')} — ${lang[1]}. Выберите одно или несколько утверждений, которые ${rand ? 'неверны' : 'верны'} при указанных условиях. В ответе запишите номера выбранных утверждений без пробелов, запятых и других дополнительных символов. Если ответов несколько, записывайте их номера в порядке возрастания.`,
