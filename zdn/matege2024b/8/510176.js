(function () {
	...
	retryWhileError(function () {
		...
		let gasOrElevator = ['установлены газовые плиты', 'не установлен лифт'][randTask];

		let correct = [
			`Если в доме `+ gasOrElevator + `, то в этом доме менее ${limitText} этажей.`,
			`Если в доме ` + gasOrElevator + `, то в нём не более ${limit} этажей.`,
			`Дома с ${limitText} и более этажами имеют `+ ['электрические плиты', 'лифт'][randTask] + `.`,
			['Газовые плиты возможны', 'Отсутствие лифта возможно'][randTask] + ` только в домах до ${limit} этажей включительно.`
		];
		let wrong = [
			`Если в доме ` + gasOrElevator + `, то в этом доме более ${limitText} этажей.`,
			`Если в доме больше ${limit + sl(2,8)} этажей, то в нём ` + gasOrElevator + `.`,
