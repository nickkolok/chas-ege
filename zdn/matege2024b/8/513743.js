(function () {
	...
	retryWhileError(function () {
		...
		let correct = [
			`Не может оказаться, что ${name} ${surname} ${chislitlx(maxBoth + 1, 'день', '$')} ходил и на ${spotName[0].ve}, и на ${spotName[1].ve}.`,
			`Было ${chislitlx(minNone, 'день', '$')}, когда ${name} ${surname} не ходил ни на ${spotName[0].ve}, ни на ${spotName[1].ve}.`,
		...
		NAtask.setTask({
			text: `${name} ${surname} был в отпуске ${chislitlx(days, 'день', '$')}  и каждый день ходил куда-нибудь гулять. ${chislitlx(spotA, 'раз', '$')} он ходил на ${spotName[0].ve} и ${chislitlx(spotB, 'раз', '$')} ходил на ${spotName[1].ve} ` +
				`(за день ${name} ${surname} мог сходить и на ${spotName[0].ve}, и на ${spotName[1].ve}, а мог никуда не ходить, но дважды в день в одно и то же место не ходил).` +
				` Выберите утверждения, которые ` + (rand ? 'неверны' : 'верны') + ` при указанных условиях вне зависимости от того, в какие дни ${name} ${surname} ходил на ${spotName[1].ve}.` +
