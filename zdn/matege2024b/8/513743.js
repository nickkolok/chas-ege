(function () {
	...
	retryWhileError(function () {
		...
		let correct = [
			...
			`Было ${chislitlx(minNone, 'день', '$')} дня, когда Андрей Сергеевич не ходил ни на ${spotName[0].ve}, ни на ${spotName[1].ve}.`,
		...
		let wrong = [
			...
			`Он ни разу не был на ${spotName[0].ve}.`
		];
		...
		NAtask.setTask({
			text: `Андрей Сергеевич был в отпуске ${chislitlx(days, 'день', '$')}  и каждый день ходил куда-нибудь гулять. ${chislitlx(spotA, 'раз', '$')} он ходил на ${spotName[0].ve} и ${chislitlx(spotB, 'раз', '$')} ходил на ${spotName[1].ve} ` +
				`(за день Андрей Сергеевич мог сходить и на ${spotName[0].ie}, и на ${spotName[1].ie}, а мог никуда не ходить, но дважды в день в одно и то же место не ходил).` +
