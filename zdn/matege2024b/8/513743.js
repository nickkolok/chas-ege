(function () {
	...
	retryWhileError(function () {
		...
		let task = [`${fullName} был в отпуске ${chislitM(days, 'день', 'дня', 'дней', '$')}  и каждый день ходил куда-нибудь гулять. ${chislitM(spotA, 'раз', 'раза', 'раз', '$')} он ходил на ${spotName[0].ve} и ${chislitM(spotB, 'раз', 'раза', 'раз', '$')} ходил на ${spotName[1].ve} ` +
		...
		let correct = [
			[`Не может оказаться, что ${fullName} ${chislitM(maxBoth + 1, 'день', 'дня', 'дней', '$')} ходил и на ${spotName[0].ve}, и на ${spotName[1].ve}.`,
			`Было ${chislitM(minNone, 'день', 'дня', 'дней', '$')}, когда ${fullName} не ходил ни на ${spotName[0].ve}, ни на ${spotName[1].ve}.`,
			`Максимум ${chislitM(maxBoth, 'день', 'дня', 'дней', '$')} он был и там, и там.`,
			`Минимум ${chislitM(minNone, 'день', 'дня', 'дней', '$')} он никуда не ходил.`,
			],
			...
			`${fullName} добирается до работы не на ${transport[0].pe}.`,
			`${fullName} добирается до работы не на ${transport[0].pe}, если сегодня ${specialDay.ie}.`
			]][randTask];
		let wrong = [
			[`Было ${chislitM(spotA, 'день', 'дня', 'дней', '$')}, когда ${fullName} ходил и на ${spotName[0].ve}, и на ${spotName[1].ve}.`,
