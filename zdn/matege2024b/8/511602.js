(function () {
	...
	retryWhileError(function () {
		...
		let correct = [
			`В доме ${female[0].re} больше этажей, чем в доме ${male[1].re}.`,
			`В доме ${female[1].re} больше этажей, чем в доме ${male[1].re}.`,
			`${male[1].ie} живёт в самом малоэтажном доме.`,
			`В доме ${male[0].re} больше этажей, чем в доме ${male[1].re}, но меньше, чем в доме ${female[0].re}.`
		];
		let wrong = [
			`Дом ${male[1].re} самый многоэтажный среди перечисленных четырёх.`,
			`В доме ${male[0].re} меньше этажей, чем в доме ${male[1].re}.`,
			`Среди этих четырёх домов есть три с одинаковым количеством этажей.`,
			`${male[0].ie} живёт в самом высоком доме.`,
			`${female[0].ie} живёт в самом высоком доме.`,
			`В доме ${male[1].re} больше этажей, чем в доме ${male[0].re}, но меньше, чем в доме ${female[0].re}.`,
			`В доме ${male[0].re} больше этажей, чем в доме ${female[0].re}, но меньше, чем в доме ${male[1].re}.`,
			`В доме ${female[1].re} меньше этажей, чем в доме ${male[1].re}, но больше, чем в доме ${male[0].re}.`
		];
