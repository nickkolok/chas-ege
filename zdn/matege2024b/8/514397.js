(function () {
	...
	retryWhileError(function () {
		...
		let correct = [
			...
			`Разница между возрастом ${name[1].re} и ${name[2].re} составляет ${chislitlx(diffAgeSecond + diffAgeFirst, `год`, `$`)}.`
		];
		let wrong = [
		...
		NAtask.setTask({
			text: `${name[0].ie} младше ${name[1].re} на ${chislitlx(diffAgeFirst, `год`, `$`)}, но старше ${name[2].re} на ${chislitlx(diffAgeSecond, `год`, `$`)}. Выберите утверждения, которые ` + (rand ? 'неверны' : 'верны') + ` при указанных условиях.` +
