(function () {
	...
	retryWhileError(function () {
		...
		let wrong = [
			`За ${cheap[0].ve} заплатили больше, чем за ${medium.ve}.`,
		...
		NAtask.setTask({
			text: `Хозяйка к празднику купила ${wordMix.joinLast(', ', ' и ')}. ${medium.ie.toZagl()} стоил` + ['', 'а', 'о'][medium.rod] + 
				` дороже ${cheap[1].re}, но дешевле ${expensive.re}, и ${cheap[0].ie} стоил` + ['', 'а'][cheap[0].rod] + ` дешевле ${medium.re}. Выберите утверждения, которые ` +
				(rand ? 'неверны' : 'верны') +
