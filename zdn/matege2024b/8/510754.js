(function () {
	...
	retryWhileError(function () {
		...
		let correct = [
			`Найдутся хотя бы ${chislitlx(minBoth,'ученик','r')} из этого класса, кто посещает оба кружка.`,
			`Не найдётся ${chislitlx(maxBoth + 1,'человек') } из этого класса, которые посещают оба кружка.`,
			`Минимальное число посещающих оба кружка — ${minBoth}.`,
			`Максимальное число посещающих оба кружка — ${maxBoth}.`,
			`Если все, кто ходит на ${subject[1].ve}, также ходят на ${subject[0].ve}, то оба кружка посещают ${chislitlx(geo,'человек')}.`,
			`Число, не посещающих ни один кружок, меньше или равно ${total - Math.max(bio, geo)}.`
		];
		let wrong = [
			`Если ученик из этого класса ходит на кружок по ${subject[0].pe}, то он обязательно ходит на кружок по ${subject[1].pe}.`,
			...
			`Точно ${chislitlx(bio + geo - total,'ученик','r')} посещают оба кружка.`,
		];

		NAtask.setTask({
			text: `В классе учится ${chislitlx(total,'человек')}, из них ${chislitlx(bio,'человек')} посещают кружок по ${subject[0].pe}, а ${geo} — кружок по ${subject[1].pe}. Выберите одно или несколько утверждений, которые ` + (rand ? 'неверны' : 'верны') +
