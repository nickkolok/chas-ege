(function () {
	...
	retryWhileError(function () {
		...
		let correct = [
			...
			`Максимум ${chislitlx(lemon,'кружка','v')} с лимоном могут быть без сахара.`,
			`Минимум ${chislitlx(minNone,'кружка','v')} ничем не дополнены.`
		];
		let wrong = [
			`Найдётся ${chislitlx(lemon,'кружка','v')} с чаем с лимоном, но без сахара.`,
		...
		NAtask.setTask({
			text: `На столе стоят ${chislitlx(total,'кружка')} с чаем. В ${sugar} из них чай с сахаром, а в остальных — без сахара. В ${lemon} из этих ${chislitlx(total,'кружка','r')} официант собирается положить по дольке лимона.` +
