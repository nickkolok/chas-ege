(function () {
	...
	retryWhileError(function () {
		...
		let correct = [
			...
			`Не более ${chislitlx(maxBoth, 'студент', 'r')} из этой группы сдали зачёты и по ${subject[0]}, и по ${subject[1]}.`,
		...
		NAtask.setTask({
			text: `В группе учится  ${chislitlx(total, 'студент')}, из них ${chislitlx(econ, 'человек')} сдал${['и', ''][Number(chislitlx(econ, 'человек').split(" ")[1] == 'человек')]} зачёт по ${subject[0]} и ${chislitlx(eng, 'человек')} сдали зачёт по ${subject[1]}. Выберите одно или несколько утверждений, которые ` + (rand ? 'неверны' : 'верны') + ` при указанных условиях. В ответе запишите номера выбранных утверждений без пробелов, запятых и других дополнительных символов. Если ответов несколько, записывайте их номера в порядке возрастания.`,
