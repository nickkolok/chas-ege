(function () {
    ...
    retryWhileError(function () {
        ...
        let time = [0].zapMonot(13, 0, 1, 1); // шкала времени
        let value = [0, sl(0.3, 0.5, 0.1), sl(0.5, 1, 0.1)]; // шкала пульса
        ...
        for (; value.length < time.length - 1;) {
            ...
            for (let j = 0; j < interI; j++) {
                let curentValue = value[value.length - 1] + sl(1, 1.5, 0.1) * (-1).pow(count);
                while (!curentValue.mzhd(1, 7, true)) {
                    if (curentValue < 1) {
                        curentValue += 0.1;
                    }
                    if (curentValue > 5) {
