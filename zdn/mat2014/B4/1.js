...
var a=sluchiz(window.tovary.rm,1)[0];
var td='</td><td>';
var tr='</td></tr><tr><td>';
var b=sluchch(0,2);
var c=['самую высокую оценку','самую низкую оценку','разницу между самой высокой и самой низкой оценкой'];
var d='Некоторая организация оценивает модели '+a+', при этом учитывается ';
var f = NLsets.alphabets.english.get_random_items(6);
var g = NLsets.alphabets.russian.get_random_items(3);
var m=[[],[],[]];
var n=[0,0,0];
var p=sluchch(4,10);
var q=[sluchch(2,4),sluchch(2,4),sluchch(2,4),sluchch(2,4),sluchch(2,4)];
var r=sluchiz([20,25,50,100],1)[0];
var i=0;
var j=0;
for(i=0;i<3;i++)
{
...
window.vopr.txt+='<br/>В таблице даны значения каждого показателя для трёх моделей '+a+'.';
window.vopr.txt+=' Определите '+c[b]+' среди представленных моделей.';
window.vopr.txt+='<br/><br/><table style="text-align:center;font:inherit;" border=1><tr><td>Модели '+a+td+t[0].toZagl()+td+t[1].toZagl()+td+t[2].toZagl()+td+t[3].toZagl()+td+t[4].toZagl()+tr;
for(i=0;i<3;i++){
