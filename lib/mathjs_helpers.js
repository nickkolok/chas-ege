var mathjs_helpers = {
	...
	testLocalExtremum : function(f,x,step){
		f = math.parse(f);
		let y = f.evaluate({x/* : math.parse('' + x)*/});
		let y_less = f.evaluate({x : math.parse(x + '-' + step).evaluate()});
		let y_more = f.evaluate({x : math.parse(x + '+' + step).evaluate()});

		console.log(y, y_less, y_more);
