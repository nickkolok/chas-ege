	...
	chas.integrate=function(href,strn,width,height,predupr){
		...
		document.write(
			'<iframe'+
				...
				' width="'+width+'"'+
				' height="'+height+'"'+
				' id="chas-intergated-iframe'+chas.integratedCount+'"'+
...
//		setTimeout(chas.resizeIframes,1024);
	}
	chas.resizeIframesOnce=function(){
//		setTimeout(chas.resizeIframes,1024);
		if(!bGecko){
			for(var i=1;i<=chas.integratedCount;i++)
				for(var h=200;h<=1e5;h+=200){
					var s='ih'+i+':'+h;
					if(window.frames[s]){
						window.frames[s]=undefined;
						document.getElementById('chas-intergated-iframe'+i).height=h;
					}
				}
		}else{
			for(var chto in window.frames){//Ага, щаз. Они неперечислимые в WebKit
				if(chto.search(/^JSON{/)+1)
					try{
						var parsed=JSON.parse(chto.substr(4));
						document.getElementById('chas-intergated-iframe'+parsed.nomer).height=
							parsed.height;
					}catch(e){}
...
/*		var frprops=Object.getOwnPropertyNames(window.frames);
		var plen=frprops.length;
		for(var i=0;i<plen;i++){
			var chto=frprops[i];
			if(window.frames[chto]!==undefined)
				if(chto.search(/^JSON{/)!=-1){
					console.log(chto);
					window.frames[chto]=undefined;
					var parsed=JSON.parse(chto.substr(4));
					document.getElementById('chas-intergated-iframe'+parsed.nomer).height=
						parsed.height;
				}
