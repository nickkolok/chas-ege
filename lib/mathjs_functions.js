...
ln.toTex = function (node, options) {
	return '\\ln{' + mathjs_helpers.wrapInTeXbracketsIfNeeded(node.args[0], options) + '}';
}
...
function forceBrackets(x){
	return x;
}

forceBrackets.toTex = function (node, options) {
...
function sqrtpow(x,y){
	return Math.pow(x,1/y);
}

sqrtpow.toTex = function (node) {
	let sqrt;
	switch (node.args[1].value) {
		case 1:
			sqrt = node.args[0].value;
			break;
		case 2:
			sqrt = '\\sqrt{' + node.args[0].value + '}';
			break;
		case node.args[1].value:
			sqrt = '\\sqrt[' + node.args[1].value + ']{' + node.args[0].value + '}';
	}
	return sqrt;
}
