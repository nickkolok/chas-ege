...
function findTriangleVertices(sideA, sideB, sideC, applicata) {
    ...
    let angleA = Math.acos((sideA * sideA + sideB * sideB - sideC * sideC) / (2 * sideA * sideB));
    let verticesC = {
        x: sideB * Math.cos(angleA),
        y: sideB * Math.sin(angleA),
        z: applicata,
    };
...
class IrregularTriangularPrism extends Prism {
    ...
    get verticesOfFigure() {
        ...
        this._vertices = findTriangleVertices(this._sideA, this._sideB, this._sideC, -0.5 * this._height);
        for (let i = 0; i < 3; i++)
            this._vertices.push({ x: this._vertices[i].x, y: this._vertices[i].y, z: 0.5 * this._height })}

        return this._vertices;
    }

    set verticesOfFigure(points) {
        this._vertices = points;
    }
}

class RectangularPrismWithRightAngledTriangleAtBase extends IrregularTriangularPrism {
    constructor({ height, sideA, sideB}) {
        super({ height: height, sideA:sideA, sideB:sideB, sideC: Math.sqrt(Math.pow(sideA,2)+Math.pow(sideB,2))});
    }
}
...
class RegularPrism extends Prism {
    ...
    set verticesOfFigure({ baseSide, height }) {
        this._baseSide = baseSide;
        this._height = height;
        this._vertices = findVerticesOfRegularPolygon(this.radiusOfCircumscribedCircle, this._numberSide, -0.5 * height);

        this._vertices.push({ x: this._vertices[this._numberSide - 1].x, y: this._vertices[this._numberSide - 1].y, z: 0.5 * this._height });
...
class Parallelepiped extends Prism {
    ...
    get verticesOfFigure() {
        if (this._vertices.length === 0) {
        this._vertices = findRectangleVertices(this._width, this._depth, -0.5 * this._height)

        this._vertices.push({ x: this._vertices[4 - 1].x, y: this._vertices[4 - 1].y, z: 0.5 * this._height })
        for (let i = 0; i < 4 - 1; i++)
            this._vertices.push({ x: this._vertices[i].x, y: this._vertices[i].y, z: 0.5 * this._height })
        }
