...
class IrregularTriangularPrism extends Prism {
    ...
    get verticesOfFigure() {
        let vertexes = findTriangleVertices(this._sideA, this._sideB, this._sideC, -0.5 * this._height);
        for (let i = 0; i < 3; i++)
    ...
    set verticesOfFigure({ sideA, sideB, sideC, height }) {
        let vertexes = findTriangleVertices(sideA, sideB, sideC, -0.5 * height);
        for (let i = 0; i < 3; i++)
            vertexes.push({ x: vertexes[i].x, y: vertexes[i].y, z: 0.5 * height })

        this.verticesOfFigure = vertexes;
    }
}
...
class RegularPrism extends Prism {
    ...
    get verticesOfFigure() {
        let vertexes = findVerticesOfRegularPolygon(this._baseSide, this._numberSide, -0.5 * this._height);

        vertexes.push({ x: vertexes[this._numberSide - 1].x, y: vertexes[this._numberSide - 1].y, z: 0.5 * this._height })
    ...
    set verticesOfFigure({ baseSide, height }) {
        let vertexes = findVerticesOfRegularPolygon(baseSide, this._numberSide, -0.5 * height);
...
class Parallelepiped extends Prism {
    ...
    set verticesOfFigure({ width, depth, height }) {
        let vertexes = findRectangleVertices(width, depth, -0.5 * height)

        vertexes.push({ x: vertexes[4 - 1].x, y: vertexes[4 - 1].y, z: 0.5 * height })
        for (let i = 0; i < 4 - 1; i++)
            vertexes.push({ x: vertexes[i].x, y: vertexes[i].y, z: 0.5 * height })
...
class RegularPyramid extends Pyramid {
    ...
    get connectionMatrix() {
        let connectionMatrix = [];
            for (let i = 1; i < this._numberSide - 1; i++) {
    ...
    set verticesOfFigure({baseSide, height}) {
        let vertexes = findVerticesOfRegularPolygon(baseSide, this._numberSide, -0.5 * height);
        vertexes.push({ x: 0, y: 0, z: 0.5 * height })
...
class RectangularPyramidWithRightAngledTriangleAtBase extends Pyramid {
    ...
    set verticesOfFigure({ sideA, sideB, sideC, height }) {
        let vertexes = findTriangleVertices(sideA, sideB, sideC, -0.5 * height);
        vertexes.push({ x: vertexes[0].x, y: vertexes[0].y, z: 0.5 * height });
...
class RectangularPyramidWithRectangleAtBase extends Pyramid {
    ...
    get verticesOfFigure() {
        let vertexes = findRectangleVertices(this._sideA, this._sideB, -0.5 * this._height);
        vertexes.push({ x: vertexes[0].x, y: vertexes[0].y, z: 0.5 * this._height });
        return vertexes;
    }

    set verticesOfFigure({ sideA, sideB, height }) {
        let vertexes = findRectangleVertices(sideA, sideB, -0.5 * height);
        vertexes.push({ x: vertexes[0].x, y: vertexes[0].y, z: 0.5 * height });
