...
function findTriangleVertices(sideA, sideB, sideC) {
    ...
    let vertexesA = { x: 0, y: 0, z: 0 };
    let vertexesB = { x: sideA, y: 0, z: 0 };
...
class Triangle {
    ...
    area() {
      return calculateTriangleArea(sideA, sideB, sideC);
    }
  }  
...
class IrregularTriangularPrism extends Prism {
    constructor({ height, sideA, sideB, sideC }) {
        let tri = new Triangle ({sideA:sideA, sideB:sideB, sideC:sideC })
        super({ height, baseArea: tri.area() });
        this.sideA = sideA;
        this.sideB = sideB;
        this.sideC = sideC;
        this.perimeterOfBase = tri.perimeter();
    ...
    coordinates(matrix) {
        let vertexes = findTriangleVertices(this.sideA, this.sideB, this.sideC);
        for (let i = 0; i < 3; i++)
            vertexes.push({ x: vertexes[i].x, y: vertexes[i].y, z: this.height })
...
class RegularPrism extends Prism {
    ...
    coordinates(matrix) {
        let vertexes = findVerticesOfShape(this.baseSide, this.numberSide);

        vertexes.push({ x: vertexes[this.numberSide - 1].x, y: vertexes[this.numberSide - 1].y, z: this.height })
...
class Parallelepiped extends Prism {
    ...
    coordinates(matrix) {
        let vertexes = findRectangleVertices(this.width, this.depth)

        vertexes.push({ x: vertexes[4 - 1].x, y: vertexes[4 - 1].y, z: this.height })
        for (let i = 0; i < 4 - 1; i++)
            vertexes.push({ x: vertexes[i].x, y: vertexes[i].y, z: this.height })
...
class RegularPyramid extends Pyramid {
    ...
    coordinates(matrix) {
        let vertexes = findVerticesOfShape(this.baseSide, this.numberSide);
        vertexes.push({ x: 0, y: 0, z: this.height })
...
class RectangularPyramidWithRightAngledTriangleAtBase extends Pyramid {
    ...
    sideSurfaceArea() {
        return (
            calculateTriangleArea(this.sideA, this.sideEdgeA, this.height) +
            calculateTriangleArea(this.sideB, this.sideEdgeB, this.height) +
            calculateTriangleArea(this.sideC, this.sideEdgeA, this.sideEdgeB)
        );
    }
    ...
    coordinates(matrix) {
        let vertexes = findTriangleVertices(this.sideA, this.sideB, this.sideC);
        vertexes.push({ x: vertexes[0].x, y: vertexes[0].y, z: this.height })
...
class RectangularPyramidWithRectangleAtBase extends Pyramid {
    ...
    sideSurfaceArea() {
        return (
            calculateTriangleArea(this.sideA, this.sideEdgeA, this.height) +
            calculateTriangleArea(this.sideB, this.sideEdgeB, this.height) +
            calculateTriangleArea(this.sideB, this.sideEdgeA, this.sideEdgeC) +
            calculateTriangleArea(this.sideA, this.sideEdgeB, this.sideEdgeC)
        );
    }
    ...
    coordinates(matrix) {
        let vertexes = findRectangleVertices(this.sideA, this.sideB);
        vertexes.push({ x: vertexes[0].x, y: vertexes[0].y, z: this.height })
