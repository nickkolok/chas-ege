...
try{
	...
	if(parsedJSON.isString)
		...
		// Поддержка переопределения chas.mode через URL hash
		if(parsedJSON.mode){
			for(var key in parsedJSON.mode){
				chas.mode[key]=parsedJSON.mode[key];
			}
			console.log('chas.mode переопределён через URL:', JSON.stringify(parsedJSON.mode));
		}

		// Поддержка установки зерна ГПСЧ (seed) через URL hash
		if(parsedJSON.seed !== undefined){
			if (typeof Math.seedrandom === 'function') {
				Math.seedrandom(parsedJSON.seed);
				console.log('Seed ГПСЧ установлен через URL:', parsedJSON.seed);
			} else {
				console.warn('Math.seedrandom не найден, seed не установлен.');
			}
		}
	}
}catch(e){
