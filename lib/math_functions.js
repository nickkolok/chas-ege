...
function createSpline({ type, minX, maxX, minY, maxY, stepForX = 1, stepForY = 1, extremumsIsInteger = { int: false, tolerance: 0.2 }, rootsIsInteger = { int: false, tolerance: 0.2 },
    ...
    extX.forEach((elem) => genAssert((elem - minX).abs() > 0.5, 'Экстремум слишком близко к левому концу по x'));
    extX.forEach((elem) => genAssert((elem - maxX).abs() > 0.5, 'Экстремум слишком близко к правому концу по x'));

    if (extremumsIsInteger.int) {
        extX.forEach((elem) => {
    ...
    extY.forEach((elem) => genAssert((elem - minY).abs() > 0.5, 'Экстремум слишком близко к левому концу по y'));
    extY.forEach((elem) => genAssert((elem - maxY).abs() > 0.5, 'Экстремум слишком близко к правому концу по y'));

    let rootF = roots(func, minX, maxX);
