...
function minimumsX(func, minX, maxX) {
    return findExtremumsOfFunction(func, minX, maxX).minP.map(point => point);
}

function maximumsX(func, minX, maxX) {
    return findExtremumsOfFunction(func, minX, maxX).maxP.map(point => point);
}
...
function paintSpline({ func, minX, maxX, minY, maxY, points = [], step = 0.01, scale = 20, height = 400, width = 500, font = "14px liberation_sans", lineWidth = 0.1, singleSegmentX = 1, singleSegmentY = 1,
    markedPoints = {
...
function paintMarkedPoints(ctx, points, func, options = {
    ...
    const { scale, lineDash, lineWidth, circleRadius } = options;
    const { type, fontMarkedPoints } = markedPoints;
    console.log('type', type);
    points.forEach((point, index) => {
        const x = point;
        const y = func(point);
        const yOffset = y < 0 ? -3 : 12;

        ctx.font = fontMarkedPoints;
        switch (type) {
            case 'number':
                ctx.fillText(x.ts(), (x + (y < 0 ? 0 : -0.5)) * scale, yOffset);
                break;
            case 'symbol':
                ctx.fillText('x' + (index + 1).toSubscript(), (x + (y < 0 ? 0 : -0.5)) * scale, yOffset);
                break;
        }
