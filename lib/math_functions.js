...
function findExtremumsOfFunctionSort(func, minX, maxX, step = 0.001, boundaryPointIsMax = false, boundaryPointIsMin = false) {
    const extremums = findExtremumsOfFunction(func, minX, maxX, step, boundaryPointIsMax, boundaryPointIsMin);
    const allExtremums = [...extremums.minP, ...extremums.maxP].flat().sort((a, b) => a - b);
...
function createSpline({ minX, maxX, stepForX = 1, minY, maxY, stepForY = 1, extremumsIsInteger = false, rootsIsInteger = false }) {
    const X = [];
    const Y = [];
    ...
    genAssert(func(minY) < maxX-1, 'Экстремум за пределами сетки');
    genAssert(func(minY) > minY-1, 'Экстремум за пределами сетки');
    ...
    if (extremumsIsInteger) {
        let extremumsOfFunc = findExtremumsOfFunctionSort(func, minX, maxX);
        console.log('Экстремумы', extremumsOfFunc);
...
try {
    ...
    global.findExtremumsOfFunctionSort = module.exports.findExtremumsOfFunctionSort = findExtremumsOfFunctionSort;
