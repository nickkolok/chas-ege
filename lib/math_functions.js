...
function intPointsWithTolerance(func, o) {
    ...
    let XY = [];
    for (let i = o.minX; i < o.maxX; i += o.step) {
        if (isCloseToInteger(func(i), o.tolerance) && func(i) <= o.maxY && func(i) >= o.minY) {
            XY.push([i, func(i)]);
        }
    }
    return XY;
...
function minimumsX(func, minX, maxX) {
    return findExtremumsOfFunction(func, minX, maxX).minP.map(point => point[0]);
}

function maximumsX(func, minX, maxX) {
    return findExtremumsOfFunction(func, minX, maxX).maxP.map(point => point[0]);
}
...
function createSpline({ type, minX, maxX, minY, maxY, stepForX = 1, stepForY = 1, extremumsIsInteger = {int: 'no_matter', tolerance: 0.2}, rootsIsInteger = {int: 'no_matter', tolerance: 0.2},
    ...
    Y.push(slKrome(0, minY + 0.1, maxY - 0.1));
    ...
    genAssert(painFunc(maxX).abs() > 1, 'Функция заканчивается нулём');
    genAssert(painFunc(minX).abs() > 1, 'Функция начинается нулём');
...
function paintSpline({ func, minX, maxX, minY, maxY, points = [],  step = 0.01, scale = 20, height = 400, width = 500, font = "12px liberation_sans", lineWidth = 0.1, singleSegmentX = 1, singleSegmentY = 1,
}) {
    return function (ctx) {
        ...
        for (let i = 0; i < points.length; i++) {
            ...
            if (func(points[i]) < 0) {
                ctx.fillText('x', points[i] * 20, -3);
                ctx.font = "14px liberation_sans";
                ctx.fillText((i + 1).toString(), (points[i] + 0.4) * 20, 0);
            } else {
                ctx.fillText('x', (points[i] - 0.5) * 20, 12);
                ctx.font = "14px liberation_sans";
                ctx.fillText((i + 1).toString(), (points[i] - 0.1) * 20, 15);
            }

            ctx.setLineDash([4, 2]);
            ctx.drawLine(points[i] * 20, 0, points[i] * 20, -func(points[i]) * 20);
            graph9AmarkCircles(ctx, [
                [points[i] * 20, 0]
            ], 1, 2);
...
function paintMarkedPoints(ctx, points, func, options = {
    scale: 20,
    ...
    const { scale, font, lineDashPattern, lineWidth, circleRadius } = options;
    console.log('points', points);
    points.forEach((point, index) => {
        const x = point[0] * scale;
        const y = -point[1] * scale;
        const labelOffset = func(point[0]) < 0 ? 0.4 : -0.1;
        const yOffset = func(point[0]) < 0 ? -3 : 12;
        const labelYOffset = func(point[0]) < 0 ? 0 : 15;

        ctx.fillText('x', x + (func(point[0]) < 0 ? 0 : -0.5) * scale, yOffset);

        ctx.font = font;
        ctx.fillText((index + 1).toString(), (point[0] + labelOffset) * scale, labelYOffset);

        ctx.setLineDash(lineDashPattern);
        ctx.lineWidth = lineWidth;
        ctx.drawLine(x, 0, x, y);

        ctx.drawFilledCircle(x, 0, circleRadius);
    });
}
