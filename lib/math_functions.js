...
function createSpline({ type, minX, maxX, minY, maxY, stepForX = 1, stepForY = 1, extremumsIsInteger = { int: false, tolerance: 0.2 }, rootsIsInteger = { int: false, tolerance: 0.2 },
    ...
    extY.forEach((elem) => genAssert((elem - minY).abs() > 1, 'Экстремум слишком близко к левому концу по y'));
    extY.forEach((elem) => genAssert((elem - maxY).abs() > 1, 'Экстремум слишком близко к правому концу по y'));
