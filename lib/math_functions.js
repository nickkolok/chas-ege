...
function brent(func, lowerBound, upperBound, tolerance = 1e-7, maxIterations = 1000) {
    ...
    if (fLower * fUpper >= 0) 
        throw new Error("Функция должна иметь разные знаки на концах интервала lowerBound и upperBound");

    let previousBound = lowerBound, fPrevious = fLower;
    let intervalWidth = upperBound - lowerBound, lastStep = intervalWidth;

    for (let iteration = 0; iteration < maxIterations; iteration++) {
        if (fUpper.abs() > fPrevious.abs()) {
            [lowerBound, upperBound, fLower, fUpper] = [upperBound, lowerBound, fUpper, fLower];
        }

        let toleranceEffective = 2 * Number.EPSILON * upperBound.abs() + tolerance;
        let midpoint = 0.5 * (previousBound - upperBound);

        if (midpoint.abs() <= toleranceEffective || fUpper === 0)
            return upperBound;

        let step, denominator;
        if (lastStep.abs() >= toleranceEffective && fLower.abs() > fUpper.abs()) {
            ...
            if (2 * step >= (lastStep * denominator).abs() || step >= (3 * midpoint * denominator - toleranceEffective * denominator).abs()) {
                step = midpoint;
                denominator = 1;
            }
        } else {
        ...
        upperBound += intervalWidth.abs() > toleranceEffective ? intervalWidth : midpoint.sign() * toleranceEffective;
    ...
    throw new Error(`Превышено максимальное количество итераций при поиске корней методои Брента: ${maxIterations} итераций. Интервал: [${lowerBound}, ${upperBound}]`);
}
...
function isCloseToInteger(value, tolerance) {
    return (value - value.round()).abs() <= tolerance;
}
...
function findRoots(func, minX, maxX, stepSize = 0.1, tolerance = 1e-7, maxIterations = 1000) {
    ...
    while (current < maxX) {
        let next = (current + stepSize, maxX).min();
        ...
        if (fCurrent * fNext <= 0) {
            try {
                ...
                if (!roots.some(r => (r - root).abs() < tolerance)) {
                    roots.push(root);
                }
            } catch (error) {
...
function findIntegerPointsInInterval(interval, minX, maxX, includeMinX = false, includeMaxX = false) {
    ...
    let start = Math.max(interval[0].ceil(), minX);
    let end = Math.min(interval[1].floor(), maxX);
...
function findIntExtremumsOfFunction(func, minX, maxX, toleranceValue = 0.2, step = 0.001, boundaryPointIsMax = false, boundaryPointIsMin = false) {
    ...
    let minP = extremums.minP.filter(extremum =>
        isCloseToInteger(extremum[0], toleranceValue)
    );

    let maxP =extremums.maxP.filter(extremum => 
        isCloseToInteger(extremum[0], toleranceValue)
    );
...
function transformExtremumsToIntervals(func, minX, maxX, endSegmentIsNotExtremum = false) {
    ...
    allExtremumsX.forEach((extremum, i) => {
        ...
        switch (i) {
            case 0:
                leftEnd = minX + 1;
                rightEnd = (allExtremumsX[i + 1] - 1).round();
                break;
            case allExtremumsX.length - 1:
                leftEnd = (allExtremumsX[i - 1] + 1).round();
                rightEnd = maxX - 1;
                break;
            default:
                leftEnd = (allExtremumsX[i] + 1).round();
                rightEnd = (allExtremumsX[i + 1] - 1).round();
        }
...
function findMinimums(func, minX, maxX) {
    return findExtremumsOfFunction(func, minX, maxX).minP;
}
...
function findMaximums(func, minX, maxX) {
    return findExtremumsOfFunction(func, minX, maxX).maxP;
}
...
function findDecreasingIntervals(func, minX, maxX) {
    return findIntervalsOfIncreaseAndDecrease(func, minX, maxX).decreasingIntervals;
}
...
function findIncreasingIntervals(func, minX, maxX) {
    return findIntervalsOfIncreaseAndDecrease(func, minX, maxX).increasingIntervals;
}
...
function findNegativeIntervals(func, minX, maxX, step = 0.001, tolerance = 1e-7, maxIterations = 1000) {
    ...
    return findPositiveAndNegativeIntervals(func, minX, maxX, roots, false, false).negativeIntervals;
}
...
function findPositiveIntervals(func, minX, maxX, step = 0.001, tolerance = 1e-7, maxIterations = 1000) {
    let roots = findRoots(func, minX, maxX, step, tolerance, maxIterations);
    return findPositiveAndNegativeIntervals(func, minX, maxX, roots, false, false).positiveIntervals;
}
...
function createSpline({ type, minX, maxX, minY, maxY, stepForX = 1, stepForY = 1, extremumsIsInteger = { int: 'no_matter', tolerance: 0.2 }, rootsIsInteger = { int: 'no_matter', tolerance: 0.2 },
    ...
    for (let index = 1; index < extY.length; index++) {
        genAssert((extY[index] - extY[index - 1]).abs() > minimumDifferenceBetweenExtremes, 'Разница между соседними экстремумами меньше, чем ' + minimumDifferenceBetweenExtremes);
    };
...
function isDistinctByTolerance(array, tolerance) {
    for (let i = 0; i < array.length; i++) {
        for (let j = i + 1; j < array.length; j++) {
            if ((array[i] - array[j]).abs() < tolerance) {
                return false;
            }
        }
    }
