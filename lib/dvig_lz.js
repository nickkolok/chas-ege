...
function joinDecorationList(decorationList) {
	...
	for (var decoration in decorationList) {
		code += '\tvar ' + decoration + '= decor.' + decorationList[decoration][0] + '.iz();\n';
		for (var i = 1; i < decorationList[decoration].length; i++){
			code +=
				'\t//var ' + decoration + ' = decor.' + decorationList[decoration][i] + '.iz();' +
				'//' + JSON.stringify(decor[decorationList[decoration][i]].slice()) +
				'\n';
		}
	}
...
function replaceDecorations_1word_1topic(lexemArray, form2word, array2word, word2array, decorationList) {
	...
	for (var word in word2array) {
		if (word2array[word].length !== 1){
			// Встретилось больше, чем в одном массиве декораций
			continue;
		}
		var array = word2array[word][0];
		...
		// Так-с. У нас одно слово, и ему однозначно находится массив декораций.
		// Сла-а-авно! *облизывается*
		console.log(word, array);
		safepushtodict(decorationList, 's' + decorationCounter, array);
		for (var j = 0; j < lexemArray.length; j++) {
			if (form2word[lexemArray[j]] != word) {
				continue;
			}
			lexemArray[j] =
				"' + s" + decorationCounter + "." + lx_guessWordForm(word, lexemArray[j]) + " +'";
		}
		decorationCounter++;
	}
}
