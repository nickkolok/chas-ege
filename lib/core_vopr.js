...
// Функция `vrn_ist` проверяет, существует ли стандартная версия данных `candidate` в массиве `correctAnswers`.
...
// Функция `vrn_mat` проверяет, совпадает ли временная метка `ts` объекта `candidate` с любым элементом в массиве `correctAnswers`.
...
// Функция `vrn_list` сравнивает список, полученный из `candidate`, с массивом `correctAnswers`, удаляя дубликаты и сортируя.
...
window.vopr.podg = function() {
    ...
    window.vopr.vrn = window.vopr.vrn_mat; // Установка функции проверки по умолчанию
}
window.vopr.podg(); // Вызов функции инициализации
