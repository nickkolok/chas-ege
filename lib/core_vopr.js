...
function AtoB(n = 3, {autoLaTeX = false, sortingFunction = null} = {}){
	...
	var options = [ver].concat(nev).shuffle();
	
	if (typeof sortingFunction === 'function') {
		options = sortingFunction(options, ver);
	}

	window.vopr.ver=[options.indexOf(ver)+1];
	window.vopr.nev=[];
	for(var i=0;i<=n;i++){
		let theAnswer = options[i];
