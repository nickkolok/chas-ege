 ...
 * Если writeNoun установлен в true, возвращает существительное вместе с изменённым глаголом.
 *
 * @param {string} verb - Глагол в прошедшем времени мужского рода.
 * @param {string} noun - Существительное в именительном падеже.
 * @returns {string} Изменённый глагол.
 * @throws {Error} Если verb или noun не определены.
 */
function selectVerbGender(verb, noun) {
	if (typeof verb === 'undefined') {
		throw new Error('Verb must be defined');
	}
	if (typeof noun === 'undefined') {
		throw new Error('Noun must be defined');
	}

	noun = sklonlxkand(noun);

	if (verb.slice(-1) == 'л') {
		return verb + ['', 'а', 'о', 'и'][noun.rod];
	}

	if (verb.slice(-2) == 'ся'){
		return verb.replace('ся', ['ся', 'ась', 'ось', 'ись'][noun.rod]);
	}

	throw new Error('The verb cannot be changed, verb: ' + verb);
}
