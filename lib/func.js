...
function autoScale(figure, camera, vertex2D, {
	...
	genAssert(verticesInGivenRange(vertex2D, {
		...
		startY: startY,
		finishY: finishY
	}), 'Фигура не помещается, уменьшите исходный масштаб');
	do {
		camera.scale += step || 0.1;
		vertex2D = figure.verticesOfFigure.map((coord3D) => project3DTo2D(coord3D, camera));
