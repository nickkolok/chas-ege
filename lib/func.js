...
function usePreference(preferenceKey, preference) {
	let pref = [];
	if (nabor.preferences && preferenceKey in nabor.preferences) {
		(nabor.preferences[preferenceKey].forEach((preferenceValue, index) =>
			preference[index].forEach((item) => {
				if (preferenceValue === item.preference) {
					pref.push(item.preferenceValue);
				};
			})));
	}
	return pref;
