...
function maximumsOfFunction(func, minX, maxX, step, minXIsMaximum, maxXIsMaximum) {
	//Возвращает массив, содержащий координаты всех максимумов функции
	//func - функция типа f(x)
	//minX,maxX - начало и конец поиска максимумов функции
	//step - шаг
	//minXIsMaximum - необходимо ли включить левую точку в максимумы
	//maxXIsMaximum - необходимо ли включить правую точку в максимумы
	let maximum = [];
	if (minXIsMaximum) {
		if (func(minX) > func(minX + step))
			maximum.push([minX, func(minX)]);
	}

	if (maxXIsMaximum) {
		if (func(maxX) > func(maxX - step))
			maximum.push([maxX, func(maxX)]);
	}

	for (let i = minX + step; i < maxX; i += step)
		if (func(i) > func(i - step) && func(i) > func(i + step))
			maximum.push([i, func(i)]);
	return maximum;
}

function minimumsOfFunction(func, minX, maxX, step, minXIsMinimum, maxXIsMinimum) {
	//Возвращает массив, содержащий координаты всех минимумов функции
	//func - функция типа f(x)
	//minX,maxX - начало и конец поиска минимумов функции
	//step - шаг
	//minXIsMaximum - необходимо ли включить левую точку в минимумы
	//maxXIsMaximum - необходимо ли включить правую точку в минимумы
	let minimum = [];
	if (minXIsMinimum) {
		if (func(minX) < func(minX + step))
			minimum.push([minX, func(minX)]);
	}

	if (maxXIsMinimum) {
		if (func(maxX) < func(maxX - step))
			minimum.push([maxX, func(maxX)]);
	}
	
	for (let i = minX + step; i < maxX; i += step)
		if (func(i) < func(i - step) && func(i) < func(i + step))
			minimum.push([i, func(i)]);
	return minimum;
}
try{
