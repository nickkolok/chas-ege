...
function roughHTML2LaTeX(str) {
	// Полная замена на вызов библиотеки html-to-latex
	try {
		const converter = typeof require !== 'undefined' ? require('html-to-latex') : (typeof window !== 'undefined' ? window.htmlToLatex : null);
		if (converter) {
			return converter(String(str));
		}
	} catch (e) {
		console.warn("html-to-latex не доступен, используется минимальный фоллбэк", e);
	}
