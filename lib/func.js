...
//@author : zer00player
//coordAxis_drawMarkPoint функция, которая рисует отметку на координатной оси (точку, засечку или ничего) с подписью.
/*
 ...
 * @param {'dot'|'line'|'nothing'} markForm Форма метки: точка, засечка или отсутствие.
...
*/
	...
	*/
	...
	//* @param {'dot'|'line'|'nothing'} markForm Форма метки: точка, засечка, отсутствие метки.
	switch (markForm) {
		case "dot":
	...
	switch (textPosition) {
		case "underAxis":
			ct.fillText(text, coord - textOffsetX, 20);
			break;
		case "overAxis":
			ct.fillText(text, coord - textOffsetX, -10);
			break;
		case "onAxis":
			ct.fillText(text, coord + 6, 5);
			break;
	}
