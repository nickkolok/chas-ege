...
function sl_forceIntegers(start, end, step) {
	if (!sluchch.forceIntegers) {
		// Если опция отключена - ничего не делаем
		return [start, end, step];
	}

	const noWay = 'Невозможно предпочесть генерацию целых чисел: ';
	const reject = 'Отказ предпочесть генерацию целых чисел: ';

	/*
	// Изначальная наивная реализация
	if (!(1/step).isAlmostInteger()) {
		console.log(noWay + 'шаг ' + step + ' не является долей единицы');
		return [start, end, step];
	}
	*/

	if (step.isAlmostInteger() && (!start.isAlmostInteger() || !end.isAlmostInteger())) {
		console.log(noWay + 'целый шаг ' + step + ' при нецелом диапазоне [' + start + ' ; ' + end + ']');
		return [start, end, step];
	}

	if (start >= 0 && end <= 1) {
		// Особенно актуально для вероятностей
		console.log(reject + 'целенаправлено генерируется число из отрезка [0 ; 1], а именно от ' + start + ' до ' + end + ' с шагом ' + step);
		return [start, end, step];
	}

	if (end.floor() < start.ceil()) {
		console.log(noWay + 'в диапазоне [' + start + ' ; ' + end + '] нет целых чисел');
		return [start, end, step];
	}

	// Внезапно, но
	// sl(2.4, 4.8, 0.3)
	// может и должно выдавать целое число 3

	if ((1/(start.ceil() - start)).isAlmostInteger()) {
		// Можно дошагать от начала до ближайшего целого
		start = start.ceil();
	}
	else {
		// Пытаемся дошагать хоть до какого-то целого
		let newStart = start;
		while (!newStart.isAlmostInteger()) {
			newStart += step;
			if (newStart >= end) {
				console.log(noWay + 'начало диапазона ' + start + ' не округляется с шагом ' + step + ' до целого');
				return [start, end, step];
			}
		}
		start = newStart;
	...
	if ((1/(end - end.floor())).isAlmostInteger()) {
		// Можно дошагать от начала до ближайшего целого
		end = end.floor();
	}
	else {
		// Пытаемся дошагать хоть до какого-то целого
		let newEnd = end;
		while (!newEnd.isAlmostInteger()) {
			newEnd -= step;
			if (newEnd <= start) {
				console.log(noWay + 'конец диапазона ' + end + ' не округляется с шагом ' + step + ' до целого');
				return [start, end, step];
			}
		}
		end = newEnd;
...
Если k опущено - от 0 до n*/
	...
	if (!s) {
		// Шаг можно опускать - тогда он равен единице
		s = 1;
	}
