 ...
 * @returns {Array} - Массив предпочтений. Пример
 * preference = [
			[{
				preference: 'minimum',
 ...
 */
function usePreference(preferenceKey, preference) {
	let pref;
	if (nabor.preferences && nabor.preferences[preferenceKey]) {
		nabor.preferences[preferenceKey].forEach((preferenceValue) => {
			if (Array.isArray(preference)) {
				preference.forEach((elem) => {
					if (preferenceValue === elem.preference) {
						pref = elem.preferenceValue;
					} else {
						pref = elem.defaultValue;
					}
				})
			} else {
				if (preferenceValue === preference.preference) {
					pref = preference.preferenceValue;
				} else {
					pref = preference.defaultValue;
				}
