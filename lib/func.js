			...
			if (Array.isArray(preference)) {
				preference.forEach((elem) => {
					if (value === elem.preference) {
						matches.push(elem.preferenceValue);
					}
				});
			} else {
				if (value === preference.preference) {
					matches.push(preference.preferenceValue);
				}
			}
		});
...
function getSelectedPreferenceFromList(key, preference, length) {
	...
	const prefObjects = preference.map((pref, index) => ({
		preference: pref,
	...
	let defaultValue;
	if (length !== undefined) {
		defaultValue = arrayOfUniqueValues(length, 0, preference.length - 1);
	} else {
		defaultValue = sl(preference.length - 1);
	}

	return getListedPreference(key, prefObjects, defaultValue);
}
