 ...
 * @param {string} nounOrRod - Существительное в именительном падеже или число, указывающее на род (0 - муж, 1 - жен, 2 - ср, 3 - мнж).
...
function selectVerbGender(verb, nounOrRod) {
	...
	nounOrRod = (1 * nounOrRod == nounOrRod) ? nounOrRod : sklonlxkand(nounOrRod).rod;

	if (verb.slice(-1) == 'л') {
		return verb + ['', 'а', 'о', 'и'][nounOrRod];
	}

	if (verb.slice(-3) == 'лся') {
		return verb.replace(/(лся)$/, ['лся', 'лась', 'лось', 'лись'][nounOrRod]);
	}

	if (verb.slice(-2) == 'ся') {
		return verb.replace(/(ся)$/, ['ся', 'ась', 'ось', 'ись'][nounOrRod]);
	}

	return verb + ['', 'ла', 'ло', 'ли'][nounOrRod];
}
