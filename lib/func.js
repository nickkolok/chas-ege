...
/**Наброски движка про перевод величин*/
	...
	window.vopr.text='Выразите '+chislitlx(koef,ishIzm[0])+' в '+lx[rezIzm[0]].pm;
	window.vopr.correctAnswers=[otv.ts()];
}
	...
	if(p.isString){
		...
		if(!bdr && p.reverse().match(/[0-9]{6,}[.,](?!0("|sir))/))
			rez+='6 и более цифр после десятичной запятой '+
				'(если так и должно быть, установите vopr.categories.bdr значение 1; ';
	}
	if(p.isArray){
