...
function problemDataChain0(o) {
/**Простейшая фараза "Найдите ..., если..."*/
	let unitsInBrackets = '', unitsAfter = '';
	if (o.units) {
		if (sl1()) {
			unitsInBrackets = ' (в ' + o.units + ')';
		} else {
			unitsAfter = ' Ответ дайте в ' + o.units + '.';
		}
	}
	return (
		decor.orderToFind.iz().toZagl() + ' ' + o.find + unitsInBrackets + ', ' +
		['если', 'если известно, что', 'зная, что'].iz() + ' ' +
		o.data.shuffle().joinLast(', ', [', а '/*, ', при этом '*/].iz() ) + '.' +
		unitsAfter
	);
}
