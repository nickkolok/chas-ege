...
function isCircleObj(obj) {
	return obj !== null && typeof obj === 'object' &&
		obj.center !== null && typeof obj.center === 'object' &&
		typeof obj.center.x === 'number' &&
		typeof obj.center.y === 'number' &&
		typeof (obj.radius !== undefined ? obj.radius : obj.r) === 'number';
}
...
function verticesInGivenRange(vertex, {
	...
	for (let i = 0; i < vertex.length; i++) {
		if (isCircleObj(vertex[i])) {
			let c = vertex[i];
			let r = (c.radius !== undefined) ? c.radius : c.r;
			if (!(c.center.x - r).mzhd(startX, finishX) ||
				!(c.center.x + r).mzhd(startX, finishX) ||
				!(c.center.y - r).mzhd(startY, finishY) ||
				!(c.center.y + r).mzhd(startY, finishY))
				return false;
		} else {
...
function autoScale(vertex, camera = {
	...
	// Circle в 3D не поддерживается: перспективная проекция может превратить круг в эллипс
	if (is3D && vertex.some(isCircleObj)) {
		throw new Error('autoScale: Circle не поддерживается в 3D-режиме (проекция может превратить круг в эллипс)');
	}

	// Initialize vertex2D if not provided
	if (vertex2D.length === 0) {
		vertex2D = vertex.map(coord => isCircleObj(coord)
			? {center: {x: coord.center.x, y: coord.center.y}, radius: (coord.radius !== undefined) ? coord.radius : coord.r}
			: {x: coord.x, y: coord.y}
		);
	}
	...
	do {
		...
		if (is3D) {
			...
			vertex2D = vertex.map(coord2D => isCircleObj(coord2D)
				? {
					center: {
						x: coord2D.center.x * camera.scale,
						y: coord2D.center.y * camera.scale,
					},
					radius: ((coord2D.radius !== undefined) ? coord2D.radius : coord2D.r) * camera.scale,
				}
				: {
...
try{
	...
	global. isCircleObj = module.exports. isCircleObj = isCircleObj ;
