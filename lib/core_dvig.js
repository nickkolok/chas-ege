...
dvig.validateVopr=function(){
	...
	for(var pole in vopr){
		var t=hasErrors(vopr[pole],vopr.categories.bdr);
		if(t)
			rez+=pole+" : "+t+'; ';
	}
	if(vopr.incorrectAnswers.includes(''))
		rez+='nev: пустой вариант; ';
	if(vopr.correctAnswers.includes(''))
		rez+='ver: пустой вариант; ';
	if(vopr.incorrectAnswers.hasDubl())
		rez+='nev: повторяющиеся варианты; ';
	var masOtv=vopr.correctAnswers.sortDelDubl().concat(vopr.incorrectAnswers.sortDelDubl());
	if(masOtv.hasDubl())
...
//Упорно пытается составить конкретное задание конкретной категории
	...
	clearInterval(dvig.intervZapros);
	try{
		//let oldText = window.vopr.text;
		console.log(window.vopr.text);
		console.log('Trying to generate...');

		do{
			nabor.upak[dvig.getzadname(kat)][nom]();
			console.log(window.vopr.text);
		}while(window.vopr.text == dvig.startxt);
		...
		zagr(nabor.resourceAddress+dvig.getzadname(kat)+'/'+nomer+'.js');
...
dvig.zadan=function(cb,kat,nom,ekz){'use strict';
	...
	try{
		...
		try{
			...
			zagr(nabor.resourceAddress+dvig.getzadname(kat)+'/'+nabor.scheduler+'.js');
...
dvig.getzadname=function(kat){
	return nabor.alternativeTasks[kat]?nabor.alternativeTasks[kat]:nabor.prefix+kat;
}
//}}Движок
