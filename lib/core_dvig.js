...
dvig.zadan=function(cb,kat,nom,ekz){'use strict';
	...
	try{
		...
		try{
			dvig.zapros(cb,kat,Object.keys(nabor.upak[dvig.getzadname(kat)]).iz());
/*			var nomerAvailableInUpak=[];
			for(var chto in nabor.upak[dvig.getzadname(kat)]){
				nomerAvailableInUpak.push(chto);
			}
			nomer=nomerAvailableInUpak.iz();
			dvig.zapros(cb,kat,nomer);
*/
		}catch(e2){
