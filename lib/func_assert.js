...
function genAssertGraphCrossesVertical(coordinate, message) {
	genAssert((f(-0.5) - (coordinate + 0.25)).abs() > 1 && (f(-0.2) - (coordinate + 0.25)).abs() > 1 && (f(-0.8) - (coordinate + 0.25)).abs() > 1, message || 'График перечёркивает ' + coordinate + ' у вертикальной оси');
}

function genAssertGraphCrossesHorizontal(coordinate, message) {
	genAssert((f(coordinate) + 0.5).abs() > 1 && (f(coordinate + 0.2) + 0.5).abs() > 1 && (f(coordinate + 0.4) + 0.5).abs() > 1, message || 'График перечёркивает ' + coordinate + ' у горизонтальной оси');
}
