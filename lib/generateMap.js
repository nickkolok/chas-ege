...
function generateMapData(centerX, centerY, settings = {}) {
	...
	if (mapData.village) {
		...
		const lakes = [];
		let attempts = 0;
		let lakePoints = settings.lakeCount || 2;
		
		const maxAttempts =  lakePoints * 10;

		for (let i = 0; i < lakePoints && attempts < maxAttempts; i++) {
			attempts++;
			...
			const lake = generateConcavePolygon(lakePoints, lakeCenter.x, lakeCenter.y, lakeRadius);
