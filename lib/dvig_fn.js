...
function fn_promezh(a){
	...
	if(a.prnz==undefined && a.prkz==undefined)
		return {
			nazv:'',
			text:'$\\left(-\\infty;\\infty \\right)$',
		}

	if(a.prnz==undefined && a.prkb)
		return{
			nazv:'открытый луч',
			text:'$\\left(-\\infty;'+a.prkz.ts()+' \\right)$',
		}

	if(a.prkz==undefined && a.prnb)
		return{
			nazv:'открытый луч',
			text:'$\\left('+a.prnz.ts()+';\\infty \\right)$',
		}
	...
	if(a.prkb&&a.prnb)
		return{
			nazv:'интервал',
			text:'$\\left('+a.prnz.ts()+';'+a.prkz.ts()+' \\right)$',
		}
...
function fn_formul(a){
	...
	var otv=[];
	if(a.minx!=undefined){
		text.push('точка минимума');
		otv.push(a.minx);
	}
	if(a.maxx!=undefined){
		text.push('точка максимума');
		otv.push(a.maxx);
	}
	if(a.miny!=undefined){
		text.push('наименьшее значение');
		otv.push(a.miny);
	}
	if(a.maxy!=undefined){
		text.push('наибольшее значение');
		otv.push(a.maxy);
	}
	var vpr=[text,otv].T().iz();
	return {
		text: (om.otvnaydite.iz().toZagl()+' '+lx[vpr[0]].ve+
			' функции $y = '+a.slag.slag0()+'$'+fn_na(a)).plusminus(),
		correctAnswers: vpr[1].plusminus(),
	}
}
