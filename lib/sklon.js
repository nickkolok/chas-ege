...
function lx_declareClarifiedPhrase(existingWord, clarifyingWord, clarifyingWordPlural){
	...
	if(clarifyingWordPlural){
		for (let p of ['im','rm','vm','dm','tm','pm',]){
			proposedScheme[p] += ' ' + clarifyingWordPlural;
		}
	}
	lx[existingWord + ' ' + clarifyingWord] = proposedScheme;
}

function lx_declareClarifiedPhraseWithDeclensionInPlural(existingWord, clarifyingWord){
	let existingScheme = lx[existingWord] || sklonlxkand(existingWord);
	let proposedScheme = existingScheme.clone();
	for (let p of ['ie','re','ve','de','te','pe']){
		proposedScheme[p] += ' ' + clarifyingWord;
	}

	let clarifyScheme = lx[clarifyingWord] || sklonlxkand(clarifyingWord);
	let clpProposedScheme = clarifyScheme.clone();

	for (let p of ['im','rm','vm','dm','tm','pm',]){
		proposedScheme[p] += ' ' + clpProposedScheme[p];
	}
	lx[existingWord + ' ' + clarifyingWord] = proposedScheme;
}
...
try{
	global. sklonlxkand = module.exports. sklonlxkand = sklonlxkand ;
} catch (e) {
}
