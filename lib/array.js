...
Array.prototype.joinWithConjunction = function() {
	/** Соединяет массив запятыми и соединяет для последних элемента союзом*/
	/** ["a=5","b=1","c=9"].joinWithConjunction() => "a=5, b=1 и c=9"*/
	return this.joinLast(" и ");
}
...
Array.prototype.randomReverse = function () {
	return sl1() ? this.slice().reverse() : this.slice();
}
...
Array.prototype.joinPlusMinus = function() {
/**Перемешивает массив в случайном порядке и радостно соединяет плюсами и минусами*/
	let join = '';
	let copy = this.slice().shuffle();
	copy.slice(0, -1).forEach((elem) => {
		join += (''+elem).replace(/^[-]/, '') + ['+', '-'].iz();
	});
	return ['', '-'].iz() + join + (''+copy.slice(-1)).replace(/^[-]/, '');
};
...
//TODO: standard Array.prototype.includes ?
...
Array.prototype.joinWithConjunction = function(separator) {
	/** Соединяет массив separator и добавляет перед последним союз*/
	/** ["a=5","b=1","c=9"].joinWithConjunction() => "a=5, b=1 и c=9"*/
	separator = separator || ', ';
	return this.slice(0, this.length - 1).join(separator) +  ' и ' + this[this.length - 1];
}
