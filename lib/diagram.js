...
function isNotMore(interval, value) {
    return interval.length > 0 && interval.every(item => item <= value);
}

function isNotLess(interval, value) {
    return interval.length > 0 && interval.every(item => item >= value);
}

// Функции для поиска экстремумов в интервалах
/**
 * Находит индекс интервала, содержащего глобальный экстремум.
 * @param {Array} intervals - массив интервалов (массивов значений)
 * @param {Array} values - массив значений для поиска экстремума
 * @param {Function} extremumFn - функция math.max или math.min
 * @returns {number|null} индекс интервала или null
 */
function findExtremumInIntervals(intervals, values, extremumFn) {
	if (!intervals.length || !values.length)
		return null;

	const globalExtremum = extremumFn(values);
	const indices = [];

	intervals.forEach((interval, i) => {
		if (interval.some(val => math.equal(val, globalExtremum))) {
			indices.push(i);
		}
	});
...
function findMaxInIntervals(intervals, values) {
	return findExtremumInIntervals(intervals, values, math.max);
}

function findMinInIntervals(intervals, values) {
	return findExtremumInIntervals(intervals, values, math.min);
}

// Функции для анализа последовательных одинаковых значений
/**
 * Находит максимальную длину последовательности элементов, равных targetValue.
 * @param {Array} interval - массив значений
 * @param {*} targetValue - искомое значение
 * @returns {number} максимальная длина последовательности
 */
function lengthOfValueInterval(interval, targetValue) {
	if (!interval.length)
		return 0;
...
function lengthOfZeroInterval(interval) {
	return lengthOfValueInterval(interval, 0);
}
...
function hasNoDuplicateValue(array, value) {
...
const isNonMore = isNotMore;
const isNonLess = isNotLess;
const noHasDublValue = hasNoDuplicateValue;
