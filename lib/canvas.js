...
CanvasRenderingContext2D.prototype.drawEllipse = function(x, y, radiusX, radiusY, rotation, startAngle, endAngle, anticlockwise) {
	anticlockwise = !!anticlockwise;
	this.beginPath();
...
CanvasRenderingContext2D.prototype.drawCoordinatePlane = function(width, height, cell, text, scale, strokeStyle) {
...
CanvasRenderingContext2D.prototype.signSegmentInMiddle = function(x1, y1, x2, y2, letter, length, maxLength) {
	...
	let middle = coordinatesMiddleOfSegment(x1, y1, x2, y2);
	let angle = ((y2 - y1) / (x2 - x1)).arctg();
	this.markSegmentWithLetter(middle[0], middle[1], angle + 0.5 * Math.PI, letter, length, maxLength);
};
