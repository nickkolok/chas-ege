...
CanvasRenderingContext2D.prototype.arcBetweenSegments = function(coordinates, radius) {
	...
	if (diff.abs() - Math.PI / 2 < 0.01) {
		this.drawLineAtAngle(x + angle1.cos() * radius, y + angle1.sin() * radius, angle1 + Math.PI / 2, (diffangle < Math.PI) ? -radius : radius);
		this.drawLineAtAngle(x + angle2.cos() * radius, y + angle2.sin() * radius, angle2 + Math.PI / 2, (diffangle < Math.PI) ? radius : -radius);
	} else
