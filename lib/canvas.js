...
CanvasRenderingContext2D.prototype.arcBetweenSegments = function (cord1, cord2) {
	let XY1 = [],
		XY2 = [];
	for (let i = 0; i < cord1.length; i += 2) {
		XY1.push({
			x: cord1[i],
			y: cord1[i + 1]
		});
	...
	let angle1 = XY1.mt_tgUnakl();
	let angle2 = XY2.mt_tgUnakl();

	let result = XY1.concat(XY2).coordinatesOfIntersectionOfTwoSegments();

	if ((angle1 - angle2).abs() < Math.PI)
		this.drawArc(result.x, result.y, 20, [angle1, angle2].minE(), [angle1, angle2].maxE());
	else
		this.drawArc(result.x, result.y, 20, [angle1, angle2].maxE(), [angle1, angle2].minE());
}
