...
CanvasRenderingContext2D.prototype.parallelepiped = function(object, dottedLine, diagonal) {

	dottedLine = dottedLine || [];
	diagonal = undefined || false;
	while (object.angle > Math.PI)
		object.angle -= Math.PI;
	if (object.angle === undefined || object.angle == Math.PI / 2 || object.angle == 3 * Math.PI / 2)
		object.angle = Math.PI / 6;

	//третий план
	if (dottedLine.includes(1))
		this.setLineDash([0.5, 0.2]);
	this.drawLine(0, 0, 0, object.height); //1
	this.setLineDash([]);

	if (dottedLine.includes(2))
		this.setLineDash([0.5, 0.2]);
	this.drawLine(object.width, 0, object.width, object.height); //2
	this.setLineDash([]);

	if (dottedLine.includes(3))
		this.setLineDash([0.5, 0.2]);
	this.drawLine(0, 0, object.width, 0); //3
	this.setLineDash([]);

	if (dottedLine.includes(4))
		this.setLineDash([0.5, 0.2]);
	this.drawLine(0, object.height, object.width, object.height); //4
	this.setLineDash([]);
	...
	if (diagonal) {
		...
		this.drawLine(0, 0, object.depth * Math.cos(object.angle) + object.width, object.depth / 2 + object.height);
		this.setLineDash([]);
		this.strokeStyle = 'black';
	}

	//второй план
	if (dottedLine.includes(5))
		this.setLineDash([0.5, 0.2]);
	this.drawLine(0, object.height, object.depth * Math.cos(object.angle), object.depth / 2 + object.height); //5
	this.setLineDash([]);

	if (dottedLine.includes(6))
		this.setLineDash([0.5, 0.2]);
	this.drawLine(object.width, object.height, object.depth * Math.cos(object.angle) + object.width, object.depth / 2 +
		object.height); //6
	this.setLineDash([]);

	if (dottedLine.includes(7))
		this.setLineDash([0.5, 0.2]);
	this.drawLine(0, 0, object.depth * Math.cos(object.angle), object.depth / 2); //7
	this.setLineDash([]);

	if (dottedLine.includes(8))
		this.setLineDash([0.5, 0.2]);
	this.drawLine(object.width, 0, object.depth * Math.cos(object.angle) + object.width, object.depth / 2); //8
	this.setLineDash([]);

	//первый план
	if (dottedLine.includes(9))
		this.setLineDash([0.5, 0.2]);
	this.drawLine(object.depth * Math.cos(object.angle), object.depth / 2, object.depth * Math.cos(object.angle), object.depth /
		2 + object.height); //9
	this.setLineDash([]);

	if (dottedLine.includes(10))
		this.setLineDash([0.5, 0.2]);
	this.drawLine(object.depth * Math.cos(object.angle) + object.width, object.depth / 2, object.depth * Math.cos(object.angle) +
		object.width, object.depth / 2 +
		object.height); //10
	this.setLineDash([]);

	if (dottedLine.includes(11))
		this.setLineDash([0.5, 0.2]);
	this.drawLine(object.depth * Math.cos(object.angle), object.depth / 2, object.depth * Math.cos(object.angle) + object
		.width, object.depth / 2); //11
	this.setLineDash([]);

	if (dottedLine.includes(12))
		this.setLineDash([0.5, 0.2]);
	this.drawLine(object.depth * Math.cos(object.angle), object.depth / 2 + object.height, object.depth * Math.cos(object
			.angle) + object.width, object.depth / 2 +
		object.height); //12
	this.setLineDash([]);
};
