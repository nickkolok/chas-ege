class Triangle {
    ...
    get medianPointAB() {
        return [this.medianAB.startPoint.coordinates, this.medianAB.endPoint.coordinates];
    }

    get medianPointBC() {
        return [this.medianBC.startPoint.coordinates, this.medianBC.endPoint.coordinates];
    }

    get medianPointCA() {
        return [this.medianCA.startPoint.coordinates, this.medianCA.endPoint.coordinates];
    }

    get medianEndPointAB() {
        return this.medianAB.endPoint.coordinates;
    }

    get medianEndPointBC() {
        return this.medianBC.endPoint.coordinates;
    }

    get medianEndPointCA() {
        return this.medianCA.endPoint.coordinates;
    }

    get medianEndPoints() {
        return {
            medianEndPointAB: this.medianAB.endPoint.coordinates,
            medianEndPointBC: this.medianBC.endPoint.coordinates,
            medianEndPointCA: this.medianCA.endPoint.coordinates
        };
    }
    ...
    get medianCoordinatesAB() {
        return this.medianAB.endPoint.coordinates;
    }

    get medianCoordinatesBC() {
        return this.medianBC.endPoint.coordinates;
    }
            ...
            type.forEach(typeABC => {
                switch (typeABC) {
                    ...
                    case 'C':
                        ...
                        break;
                    case 'S':
                        addV = new Array(this.connectionMatrix.length+2).fill(0);
                        addV.push(1);
                        break;
                    default:
                        throw new Error("Invalid type specified. Available types are 'A', 'B', and 'C'. " + typeABC);
                }
                this.#connectionMatrix.push(addV);
            });
        }
    }
