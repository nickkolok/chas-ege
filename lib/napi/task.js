...
NApi.task = {
	...
	setTask : function(o) {
		...
		switch (NLib.getTypeOf(answers)) {
		case "[object Array]":
			for (var i = 0; i < answers.length; i++) {
				switch (NLib.getTypeOf(answers[i])) {
				case "[object Number]":
					answers[i] = answers[i].ts().toString();
					break;
				case "[object String]":
					break;
				default:
					throw TypeError("Параметр answers должен содержать только строки и числа");
				}
			}
			break;
		case "[object Number]":
		...
		switch (NLib.getTypeOf(wrongAnswers)) {
		case "[object Array]":
			for (var i = 0; i < wrongAnswers.length; i++) {
				switch (NLib.getTypeOf(wrongAnswers[i])) {
				case "[object Number]":
					wrongAnswers[i] = wrongAnswers[i].ts().toString();
					break;
				case "[object String]":
					break;
				default:
					throw TypeError("Параметр wrongAnswers должен содержать только строки и числа");
				}
			}
			break;
		case "[object Number]":
			wrongAnswers = [wrongAnswers.ts()];
			break;
		case "[object String]":
			wrongAnswers = [wrongAnswers];
			break;
		default:
			throw TypeError("Параметр wrongAnswers должен быть строкой или числом или массивом строк и/или чисел");
		}
