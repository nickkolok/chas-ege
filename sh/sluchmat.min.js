...
function veroyatn(){for(var a,b=1;b<=nabor.numberOfTasks;b++)a=umka.verno[b]/umka.vsego[b],$("#pB"+b).html(4<umka.vsego[b]?(100*a).toFixedLess(0)+"%":""),$("#pravB"+b).html(umka.verno[b]),$("#vsegB"+b).html(umka.vsego[b]),umka.kvoNaVremya[b]&&$("#vremyaB"+b).html((umka.vremya[b]/umka.kvoNaVremya[b]).round().toDvoet()),$("#pB"+b).css("color",0.5<a?cvetMezhdu(zhelt,zelen,2*a-1):cvetMezhdu(krasn,zhelt,2*a));a=umka.verno.sum()/umka.vsego.sum();$("#pB").html(4<umka.vsego.sum()?(100*(umka.verno.sum()/umka.vsego.sum())).toFixedLess(0)+
"%":"");$("#pravB").html(umka.verno.sum());$("#vsegB").html(umka.vsego.sum());$("#pB").css("color",0.5<a?cvetMezhdu(zhelt,zelen,2*a-1):cvetMezhdu(krasn,zhelt,2*a))}var slvopr;function obnov(a){slvopr=a;$("#pole").html(slvopr.text);slvopr.trd();MathJax.Hub.Typeset();$("#otvet").html(slvopr.ver.join(";;"));$("#never").html(slvopr.nev.join(";;"));vremyaStart=(new Date).getTime()}
function vybrZad(){for(var a=1;a<=nabor.numberOfTasks;a++)v[a]=$("#cB"+a).is(":checked")?1:0;for(var b=[],a=1;a<=nabor.numberOfTasks;a++)v[a]&&b.push(a);if(b.length){if($("#radio-sluch").prop("checked"))return b.iz();if($("#radio-porad").prop("checked")){for(var c=n?n:0,a=0;b[a]<=c;a++);return b[a]?b[a]:b[0]}if($("#radio-umka").prop("checked")){b=[];for(a=1;a<=nabor.numberOfTasks;a++)if(v[a]&&5>umka.vsego[a])return a;for(a=1;a<=nabor.numberOfTasks;a++)v[a]?b.push(1.1-umka.verno[a]/umka.vsego[a]):b.push(0);return b.sVeroyatn()+1}return b.iz()}$("#pole").html('Хотя бы один тип заданий должен быть выбран! <button onclick="vybrv();">Выбрать все</button>')}
...
((new Date).getTime()-vremyaStart)/1E3,umka.kvoNaVremya[n]++);vopr.solution&&(e+="<br/><br/>"+vopr.solution);$("#protv").html(e);MathJax.Hub.Typeset();$("#prov").hide();$("#sozd").show();$("#podob").show();sohrUmka();veroyatn();specCounter("sluch");flProv=1}function uchetPrav(a,b){if(!svinta){var c=document.createElement("iframe");c.src="../sh/sluchcounter.html?"+nabor.name+"/"+a+"#"+b;c.style.display="none";c.style.top="-9999px";c.style.position="absolute";document.body.appendChild(c)}}var v=[];
function vybrv(){for(var a=1;a<=nabor.numberOfTasks;a++)$("#cB"+a).not(":checked").click()}function obrabNaVremya(){$("#check-na-vremya").is(":checked")?$(".vremya").show():$(".vremya").hide()}function obrabPraviln(){$("#check-praviln").is(":checked")?$(".praviln").show():$(".praviln").hide()}function vybr0(){for(var a=1;a<=nabor.numberOfTasks;a++)$("#cB"+a).removeAttr("checked")}var strelkaEst=1;
...
var startShell=function(){sozdGalki();zagrUmka();veroyatn();$(obrabNaVremya);$(obrabPraviln);strelkaDvig();$("#prov").hide();$("#podob").hide();galkiKat("#galki_kat","sluch");spoiler();$("#otv").keyup(function(a){if(nabor.allowMultilineAnswer||flProv)return!0;if(13==a.keyCode)return prover(),!1});assertCheckability()};
